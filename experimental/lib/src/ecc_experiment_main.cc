...
void Foo() {
  ...
  EccExperiment experiment("output_filename");
  int first_nerrors = 0, second_nerrors = 0;
  for (int iteration = 0; iteration < FLAGS_iters; iteration++) {
    experiment.Run(&first_nerrors, &second_nerrors);
    // std::cout << first_nerrors << " " << second_nerrors << std::endl;
