goog.provide('cryptogram.codec.binature');
 ...
 */
cryptogram.codec.binature = function() {
  this.blockSize = 1;
  this.quality = .04
};

cryptogram.codec.binature.octal_symbol_thresholds = [255, 0];

goog.inherits(cryptogram.codec.binature, cryptogram.codec);

cryptogram.codec.binature.prototype.logger = goog.debug.Logger.getLogger('cryptogram.codec.binature');


/** @inheritDoc */
cryptogram.codec.binature.prototype.name = function() {
  return "quadrant";
};


/** @inheritDoc */
cryptogram.codec.binature.prototype.processImage = function(img) {
};

cryptogram.codec.binature.prototype.set_pixel = function(x, y, r, g, b) {
...
cryptogram.codec.binature.prototype.set_block = function(x_start, y_start, level) {
  var r = level;
  var g = level;
  var b = level;
...
cryptogram.codec.binature.prototype.setImage = function(img) {
  this.img = img;
};

cryptogram.codec.binature.prototype.loadThumbnail = function() {
...
cryptogram.codec.binature.prototype.encode = function(data, 
  ...
  function add_char(ch,values) {
    ...
    //Hack: divide by 4 for range 0…1 
    x = Math.floor(x / 4);
    y = Math.floor(y / 4);
    values.push(x);
    values.push(y);
  ...
  for (var y = 0; y < header_height; y += block_height) {
    for (var x = 0; x < header_width; x += block_width) {
      ...
      if (value_idx < n_header_values) {
        level =  cryptogram.codec.binature.octal_symbol_thresholds[header_values[value_idx]];
      }
  ...
  for (var i = 0; i < n_values; i++) {
    octal = values[i];
    level = cryptogram.codec.binature.octal_symbol_thresholds[octal];
...
/** 
 */
cryptogram.codec.binature.prototype.getHeader = function(img, imageData) {
 ...
 * @private
 */
cryptogram.codec.binature.prototype.getBase8Value = function(img, imgData, x, y) {
...
/** @inheritDoc */
cryptogram.codec.binature.prototype.decodeProgress = function() {
  return this.y / this.img.height;
  
}

cryptogram.codec.binature.prototype.decode = function(img, imageData) {
  this.count = 0;
  this.chunkSize = 10;
  this.y = 0;
  this.img = img;
...
cryptogram.codec.binature.prototype.getChunk = function() {
